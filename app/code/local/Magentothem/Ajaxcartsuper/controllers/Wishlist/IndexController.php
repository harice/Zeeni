...
class Magentothem_Ajaxcartsuper_Wishlist_IndexController extends Mage_Wishlist_IndexController
{
    ...
    public function removeAction()
    {
          ...
          if ($this->getRequest()->getParam('callback')) {
			...
			$count_wishlist =  Mage::helper('ajaxcartsuper/data')->getTotalWishlist(); 
		
			$no_wishlist = null; 
			if($count_wishlist == 0 || !$count_wishlist) { 
					$no_wishlist.= '<span class="top_link_wl fa fa-heart-o"><small>0</small></span> ';
					$no_wishlist.= 	'<div class="top-wishlist-content">';
					$no_wishlist.='<p class="empty">'.$this->__('You have no items in your wishlist.').'</p></div>';
		

			}

			$ajaxData['count_wishlist'] = $count_wishlist;	
			$ajaxData['zero_wishlist'] = $no_wishlist;
            $ajaxData['status'] = 1;
            $ajaxData['wishlist_sidebar'] = $sidebarWishlist;
            $ajaxData['type_sidebar'] = 'wishlist';
            $ajaxData['top_link'] = $toplink;
