...
$_lof_active = Mage::getStoreConfig('sales/minimum_order/low_order_fee_active', $store->getId());    
      if (!$address->validateMinimumAmount() && $_lof_active ) {
        ...
        $tax = round($address->getTaxAmount(), 2);
        $shipping = round($address->getShippingAmount(), 2);

        // echo "******";
        // echo $address->getSubtotal() + " " + $amount + " " + $tax + " " + $shipping;

        $grandTotal =  $address->getSubtotal() + $amount + $tax + $shipping;
