...
class Mango_Loworderfee_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function sales_quote_collect_totals_after(Varien_Event_Observer $observer) {
        ...
        foreach ($quote->getAllAddresses() as $address) {
            ...
            $validateQty = Mage::getStoreConfig('sales/minimum_order/low_order_fee_qty', $store->getId());
            if($validateQty && $address->getItemQty()<$validateQty ){
               $grandTotal +=$amount;
            }

            $address->setGrandTotal($grandTotal /* + $address->getTaxAmount()  + $amount*/);
            $address->setBaseGrandTotal($grandTotal /* + $address->getBaseTaxAmount() + $amount*/);
