...
function initQuickButton(element) {
    ...
    $jq(element + ' a.product-image').each(function(path) {
        ...
        if(rel == null) {
            ...
            paths[count] = subpath;
            $jq(this).closest("." + MA.QuickView.PARENT_ELEMENT).find("." + MA.QuickView.CHILDREN_ELEMENT).append('<li class="qv-button-container"><a type="button" ' +
                'class="qv-e-button fa fa-eye btn-quickview-' + count + '">' +
                '<em>Quick View</em></a></li>');
